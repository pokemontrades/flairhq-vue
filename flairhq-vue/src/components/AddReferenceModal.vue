...
async function submit() {
  ...
  try {
    ...
    if (res.status === 400) throw new Error('A reference with that URL already exists.')
  ...
  <BaseModal :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)" max-width="560px">
    ...
    <template #body>
      <div class="field">
        ...
        <input id="ref-url" v-model="form.url" type="url" class="field-input" placeholder="https://reddit.com/r/pokemontrades/comments/…" />
      </div>
